class MaterialCategoriesController < ApplicationController
  ...
  def index
    @material_categories = MaterialCategory.paginate(:page => params[:page], :per_page => 50, :order => "name")
  ...
  def create
    ...
    respond_to do |format|
      if @material_category.save
        format.html { redirect_to(@material_category, :notice => 'MaterialCategory was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @material_category.update_attributes(params[:material_category])
        format.html { redirect_to(@material_category, :notice => 'MaterialCategory was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_back_or_default(material_categories_url) }
      format.xml  { head :ok }
    end
  end
end
