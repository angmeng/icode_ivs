class ProductMovementsController < ApplicationController
  ...
  def search
    if params[:search]
      ...
      @movements = ProductMovement.all(:conditions => ["product_id = ?", params[:search][:product_id].to_i], :order => "movement_date, id")
      ...
      @movements ||= ProductMovement.all(:conditions => ["product_id = ?", session[:product_id].to_i], :order => "movement_date, id")
      @filters ||= ProductMovement.calculate_balance(@movements, @product)
      @product_movements ||= @filters.paginate(:page => params[:page], :per_page => 50)
    end
  end
