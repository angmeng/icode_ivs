class ProductsController < ApplicationController
  ...
  def index
    ...
    @search.active_equals = true
    @products = @search.all(:include => [:product_category, :product_group], :order => "code").paginate(:page => params[:page], :per_page => 50)
  end
  ...
  def destroy
    ...
    if @product.verify_for_destroy
      flash[:notice] = "Product has been suspend"
    else
      flash[:error] = "Product having error when suspending"
    end
