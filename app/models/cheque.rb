class Cheque < ActiveRecord::Base
  ...
  def self.get_current_date_own_cheque
    self.all(:conditions => ["cheque_type_id = ? and cheque_date = ?", Setting::SUPPLIER_PAYMENT, Date.today], :order => "cheque_date")
  end
  
  def self.get_current_date_customer_cheque
    self.all(:conditions => ["cheque_type_id = ? and cheque_date = ?", Setting::CUSTOMER_PAYMENT, Date.today], :order => "cheque_date")
  end
