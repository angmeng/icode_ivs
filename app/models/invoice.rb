class Invoice < ActiveRecord::Base
  ...
  named_scope :get_sales_from, lambda {|from, to| {:conditions => ["invoice_date IN (?)", from..to]}}
  ...
  def self.check_quantity(invoice_item_attributes)
    ...
    invoice_item_attributes.each do |attributes|
      unless attributes[:quantity].blank? or attributes[:quantity].index(/[abcdefghijklmnopqrstuvwxyz]/) or attributes[:unit_price].blank? 
        ...
        st = Stock.first(:conditions => ["product_id = ? and store_location_id = ?", attributes[:product_id].to_i, attributes[:store_location_id].to_i])
  ...
  def receive_items(product)
     product.each do |product_id, content|
      unless invoice_items.first(:conditions => ["product_id = ?", product_id.to_i]) or content[:quantity].blank? or content[:quantity].index(/[abcdefghijklmnopqrstuvwxyz]/) 
  ...
  def invoice_item_attributes=(invoice_item_attributes)
    invoice_item_attributes.each do |attributes|
      unless attributes[:quantity].blank? or attributes[:quantity].index(/[abcdefghijklmnopqrstuvwxyz]/) 
        ...
        cp = CustomerPricing.first(:conditions => ["customer_id = ? and product_id = ?", customer_id, attributes[:product_id].to_i,])
  ...
  def generate_commission
    invoice_items.each do |i|
      comm = Commission.first(:conditions => ["salesman_id = ? and product_id = ?", salesman_id, i.product_id])   
