class StockTransfer < ActiveRecord::Base
  ...
  def check_quantity
    ...
    if quantity_condition <= 0  
      ...
      stock = Stock.first(:conditions => ["product_id = ? and store_location_id = ?", product_id, from_store_location_id])
      if stock 
  ...
  def increase_stock
    st = store_location.stocks.first(:conditions => ["product_id = ?", product_id])
    if st
    ...
    st2 = from_store.stocks.first(:conditions => ["product_id = ?", product_id])
